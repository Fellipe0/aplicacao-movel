...
import back.api.model.response.ResponseAnexo;
...
public abstract class RestControllerUtil {
	...
	public static Response<ResponseAnexo> imprimir(byte[] bytes, String nome, String tipo) {
		Response<ResponseAnexo> response = new Response<>();
		try {
			if(bytes == null) {
				return response;
			}
			ResponseAnexo anexo = new ResponseAnexo(nome, tipo, "data:application/" + tipo + ";base64,"
					+ new String(Base64.getEncoder().encode(bytes)));
			response.setData(anexo);
			return response;
		} catch (Exception e) {
